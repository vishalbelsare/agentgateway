...
use phonenumber::{parse, Mode, country, Metadata, PhoneNumber};
...
impl PhoneRecognizer {
    pub fn new() -> Self {
        ...
        let r: Regex = Regex::new(r#"(?:[(\[（［+＋][-x‐-―−ー－-／  ­​⁠　()（）［］.\[\]/~⁓∼～]{0,4}){0,2}\d{1,20}(?:[-x‐-―−ー－-／  ­​⁠　()（）［］.\[\]/~⁓∼～]{0,4}\d{1,20}){0,20}(?:;ext=(\d{1,20})|[  \t,]*(?:e?xt(?:ensi(?:ó?|ó))?n?|ｅ?ｘｔｎ?|доб|anexo)[:\.．]?[  \t,-]*(\d{1,20})#?|[  \t,]*(?:[xｘ#＃~～]|int|ｉｎｔ)[:\.．]?[  \t,-]*(\d{1,9})#?|[- ]+(\d{1,6})#)?"#).unwrap();
    ...
    fn recognize(&self, text: &str) -> Vec<RecognizerResult> {
        ...
        for &region in &self.regions {
            ...
            for start in 0..text.len() {
                for end in (start+7)..=std::cmp::min(text.len(), start+20) { // phone numbers are usually 7-20 chars
