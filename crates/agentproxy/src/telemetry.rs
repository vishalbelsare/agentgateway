...
pub fn set_level(reset: bool, level: &str) -> Result<(), Error> {
    if let Some(handle) = LOG_HANDLE.get() {
        ...
        let new_directive = if let Ok(current) = handle.with_current(|f| f.filter().to_string()) {
            if reset {
                if level.is_empty() { default_filter().to_string() } else { format!("{},{}", default_filter(), level) }
            } else {
...
impl<'writer> FormatFields<'writer> for IstioFormat {
    fn format_fields<R: tracing_subscriber::field::RecordFields>(
        ...
        let mut visitor = Visitor { writer, res: Ok(()), is_empty: true };
  ...
  N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(&self, ctx: &FmtContext<'_, S, N>, mut writer: Writer<'_>, event: &Event<'_>) -> std::fmt::Result {
        ...
        write!(writer, "\t{}\t", meta.level().to_string().to_ascii_lowercase())?;
  ...
  N: for<'writer> FormatFields<'writer> + 'static,
{
      ...
      S: Subscriber + for<'a> LookupSpan<'a>,
    {
        ...
        let mut write = || {
            ...
            let mut v = JsonVisitory { serializer, state: Ok(()) };
            event.record(&mut v);
...
impl<'a> FormatFields<'a> for IstioJsonFormat {
    ...
    fn add_fields(&self, _current: &'a mut FormattedFields<Self>, _fields: &Record<'_>) -> fmt::Result {
