...
impl Gateway {
	pub(super) fn new(pi: Arc<ProxyInputs>, drain: DrainWatcher) -> Gateway {
		let connector = if pi.cfg.backend_mesh {
			hbone::HBONEConnector::new(pi.store.clone(), &pi.cfg, pi.local_workload_information.clone())
		} else {
	...
	pub(super) async fn run(self) {
		...
		let mut handle_bind = |js: &mut JoinSet<anyhow::Result<()>>, b: Event<Arc<Bind>>| {
			...
			let task = js.spawn(
				Self::run_bind(self.upstream.clone(), self.pi.clone(), self.drain.clone(), b.clone()).in_current_span(),
			);
	...
	pub(super) async fn run_bind(
		...
		let Bind { name, address: _, listeners: _ } = &*b;
	...
	async fn proxy(
		...
		let proxy = HTTPProxy { upstream, bind_name, inputs, selected_listener, target_address };
	...
	async fn maybe_terminate_tls(
		...
		let acceptor = tokio_rustls::LazyConfigAcceptor::new(rustls::server::Acceptor::default(), Box::new(inner));
	...
	async fn serve_connect(
		...
		let resp = req
			...
			.await
			.unwrap();
		let con = agent_hbone::RWStream { stream: resp, buf: bytes::Bytes::new() };

		let _ = Self::proxy(upstream, bind_name, pi, None, Socket::from_hbone(ext, hbone_addr, con)).await;
	}
}
...
fn apply_request_filters(filters: &[RouteFilter], path_match: &PathMatch, req: &mut Request) -> Option<Response> {
...
fn select_best_route(
	...
	if matches!(listener.protocol, ListenerProtocol::HBONE) && listener.routes.is_empty() {
		...
		let svc = inp
			...
			.read_discovery()
			.services
			.get_by_vip(&NetworkAddress { network, address: dst.ip() })
			.unwrap();
		...
		return Some((Arc::new(default_route), PathMatch::PathPrefix(strng::new("/"))));
	}
	listener
		.routes
		.iter()
		...
		.filter_map(|(r, hostname_score)| {
			...
			let scores = r
				...
				.filter_map(|m| {
					...
					let mut score = (hostname_score, false, 0, false, 0, 0, std::cmp::Reverse(r.name.clone()));
...
mod revproxy {
	...
	fn get_upstream_addr(forward_uri: &str) -> Result<SocketAddr, ProxyError> {
		let forward_uri: hyper::Uri = forward_uri
			.parse()
			.map_err(|e| ProxyError::UpstreamError(format!("parsing forward_uri as a Uri: {e}").to_string()))?;
		let host = forward_uri
			.host()
			.ok_or(ProxyError::UpstreamError("forward_uri has no host".to_string()))?;
		let port = forward_uri
			.port_u16()
			.ok_or(ProxyError::UpstreamError("forward_uri has no port".to_string()))?;
		format!("{host}:{port}")
			.parse()
	...
	pub async fn call<T: Connect + Clone + Send + Sync + 'static>(
		...
		if request_upgrade_type.is_none() {
			...
			*request.uri_mut() = request_uri.clone();
			...
			let client = if request.version() == http::Version::HTTP_2 { h2_client } else { h1_client };
		...
		let upstream_request = Request::from_parts(request_parts.clone(), Empty::<hyper::body::Bytes>::new());
	...
	impl<T: Connect + Clone + Send + Sync + 'static> ReverseProxy<T> {
		...
		pub async fn call(
			...
			call::<T>(client_ip, forward_uri, request, &self.h1_client, &self.h2_client).await
		}
	}
}
...
fn load_balance(
	...
	let endpoints = svc.endpoints.iter().filter_map(|ep| {
		...
		if target_port.unwrap_or_default() == 0 && !ep.port.contains_key(&svc_port) {
			...
			trace!("filter endpoint {}, it does not have service port {}", ep.workload_uid, svc_port);
			return None;
		}
		if let Some(o) = override_dest {
			if !wl.workload_ips.contains(&o.ip()) {
				// We ignore port, assume its a bug to have a mismatch
				trace!("filter endpoint {}, it was not the selected endpoint {}", ep.workload_uid, o);
				return None;
			}
		}
		Some((ep, wl))
...
impl HTTPProxy {
	pub async fn proxy(&self, connection: Arc<Extension>, req: ::http::Request<Incoming>) -> Response {
		self.proxy_internal(connection, req)
			.await
	...
	async fn proxy_internal(
		...
		let (selected_route, path_match) =
			select_best_route(inputs.clone(), self.target_address, selected_listener.clone(), &req)
		...
		if let Some(resp) = apply_request_filters(selected_route.as_ref().filters.as_slice(), &path_match, &mut req) {
			return Ok(resp);
		}
		let mut mirrors = get_mirrors(selected_route.as_ref().filters.as_slice());
		let selected_backend = select_backend(selected_route.as_ref(), &req).ok_or(ProxyError::NoValidBackends)?;
		if let Some(resp) = apply_request_filters(selected_backend.filters.as_slice(), &path_match, &mut req) {
			return Ok(resp);
		}
		...
		for mirror in mirrors {
			if !rand::rng().random_bool(mirror.percentage) {
				trace!("skipping mirror, percentage {} not triggered", mirror.percentage);
				continue;
			}
		...
		let dest = match &selected_backend.backend {
			Backend::Service(svc_key) => {
				...
				let (ep, wl) = load_balance(inputs.clone(), svc.as_ref(), selected_backend.port, override_dest)
		...
		let (call, body_timeout) = if let Some(timeout) = timeout {
			let deadline =
				tokio::time::Instant::from_std(connection.get::<TcpConnectionInfo>().unwrap().start + timeout);
	...
	let dest = match &mirror.backend {
		Backend::Service(svc_key) => {
			let svc = inputs
				...
				.ok_or(ProxyError::ServiceNotFound)?;
			let (ep, wl) =
				load_balance(inputs.clone(), svc.as_ref(), mirror.port, None).ok_or(ProxyError::NoHealthyEndpoints)?;
